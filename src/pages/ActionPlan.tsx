...
import { actionPlan, organization } from '../content/publicContent'
  ...
  return (
    <PublicLayout>
      <PageIntro
        ...
        index="02"
        title="Seven priorities. One national direction."
        text={`${organization.motto} becomes a practical civic roadmap across institutions, unity, education, digital responsibility and public welfare.`}
      ...
      <section className="dpo-section dpo-section--paper">
        <div className="dpo-container dpo-plan-overview">
          <div className="dpo-plan-overview__copy">
            <SectionHeader eyebrow="The Framework" title="From shared values to measurable public action." text="Each priority supports the next. Strong institutions need responsible citizens; responsible citizens need awareness, unity and opportunity." />
          ...
          <Reveal direction="right" className="dpo-plan-overview__poster"><img src="/dpo-assets/front-2.png" alt="Client-provided DPO seven point action plan poster" /><span>Official action plan reference</span></Reveal>
        </div>
      </section>
            ...
            {actionPlan.map(({ title, text, icon: Icon }, index) => (
              <StaggerItem key={title}>
                <article className={index % 2 ? 'is-even' : ''}>
                  ...
                  <div><span>Priority {index + 1}</span><h3>{title}</h3><p>{text}</p><small><CheckCircle2 size={14} /> Awareness, engagement and accountable delivery</small></div>
                </article>
              </StaggerItem>
            ))}
          </Stagger>

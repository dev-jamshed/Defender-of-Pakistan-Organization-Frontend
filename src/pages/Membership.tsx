...
import { cardDesign, membership, organization } from '../content/publicContent'
...
export default function Membership() {
  return (
    <PublicLayout>
      <PageIntro eyebrow="Membership" index="03" title="Belong to something built on service." text="Understand the membership types, required documents, terms, fee and approval journey before submitting your application." image="/dpo-assets/home-mission-v2.jpg" />

      <section className="dpo-membership-summary-bar">
        <div className="dpo-container">
          <div><UserPlus size={22} /><span>Membership types</span><b>{membership.types.length} pathways</b></div>
          <div><BadgeCheck size={22} /><span>Membership fee</span><b>{membership.fee}</b></div>
          <div><CalendarClock size={22} /><span>Validity</span><b>{membership.duration}</b></div>
          <div><FileText size={22} /><span>Required documents</span><b>{membership.documents.length} items</b></div>
        </div>
      </section>
        ...
        <div className="dpo-container dpo-application-layout">
          <div className="dpo-application-layout__copy">
            ...
            <div className="dpo-application-tabs">
              <Reveal><article><FileCheck2 size={21} /><div><h3>Required documents</h3><DetailList items={membership.documents} /></div></article></Reveal>
              <Reveal delay={0.1}><article><FileText size={21} /><div><h3>Application information</h3><DetailList items={membership.formFields} /></div></article></Reveal>
            </div>
          </div>
      ...
      <section className="dpo-section dpo-section--paper">
        <div className="dpo-container">
          ...
          <Stagger className="dpo-terms-grid">
            {membership.terms.map((term, index) => <StaggerItem key={term}><article><span>0{index + 1}</span><ShieldCheck size={20} /><p>{term}</p></article></StaggerItem>)}
          </Stagger>
        </div>
      </section>
...
export function MemberCard() {
  return (
    <div className="dpo-member-card">
      ...
      <header><img src="/dpo-assets/logo-transparent.png" alt="" /><div><b>Defenders of Pakistan</b><span>Organization</span></div><small>MEMBER</small></header>
      ...
      <footer><span>{organization.motto}</span><BadgeCheck size={20} /></footer>
    </div>
  )
}
